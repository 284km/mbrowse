...
| `font/NotoSans-Regular.ttf` | googlefonts/noto-fonts, SIL OFL 1.1 | SHA — the one font both sides of the layout gate measure with |
| `font/widths.{cases,expected}` | `canvas.measureText` over that font | not a corpus: an oracle, generated by `gen_font_expected.py` |
...
## Why a font is vendored

A line of text is as tall as the font says it is. The layout expectations were first taken with
whatever font the browser defaulted to, which on the machine that generated them was a system font
that cannot be redistributed and that nothing here can read — so the 18px line in those numbers came
from a file not in this repository, and no engine here could ever have matched it.

Now the generator injects `font/NotoSans-Regular.ttf` into every document and waits for
`document.fonts.ready` before measuring, and the line is 22px on both sides for the same reason.
The waiting matters: a probe that measures first gets the fallback font's metrics and reports them
as if they were the vendored font's.

Two consequences worth being explicit about. The documents no longer exercise a reader's default
font — that is the trade, and a comparison against a font nobody has is not a comparison. And the
stylesheet is appended AFTER the document, never before: a `<style>` in front of the doctype makes
the doctype stray, which puts the browser in quirks mode and moves numbers that have nothing to do
with fonts.